...
| **What it opens** | Markdown (rendered, editable), a curated set of code files (highlighted like an editor, with line numbers, editable), CSV/TSV (as tables), and PDF (the browser's native viewer, read-only). A file with no extension, such as a `Makefile`, a `LICENSE` or a script named `bulk_read`, is listed too and judged when you click it: text opens, with highlighting from its shebang line, and a binary is refused with a message. |

...
echo ""
echo "Both commands read the value from a file rather than the command line, so"
echo "neither secret lands in your shell history. The password file is written"
echo "without a trailing newline, which a redirect preserves exactly."
echo ""
...
echo "  gh secret set READER_SIGNING_P12_PASSWORD --repo OWNER/REPO < \"$PASSWORD_OUT\""
echo ""
